...
function Catalog({ navigate }: { navigate: (path: string) => void }) {
  ...
  return <div className="e54-catalog">{products.map((product) => <article className="e54-card" key={product.number}><img src={product.photoUrls[0]} alt={product.name} /><div><p className="eyebrow">{product.category}</p><h3>{product.name}</h3><p>{product.variant}</p><p className="e54-description">{product.description}</p><strong>{product.price.toLocaleString("id-ID")} {product.unit}</strong><div className="e54-card-actions"><button onClick={() => navigate(`/showcase/${data.slug}/product/${product.number}`)} aria-label={product.name}>{data.hero.secondaryAction}</button>{data.contactPersonWhatsapp && !product.stockOut && <a href={`https://wa.me/${data.contactPersonWhatsapp}?text=${encodeURIComponent(product.name)}`} target="_blank" rel="noreferrer">{data.hero.primaryAction}</a>}</div></div></article>)}</div>;
}

export default function Example54({ navigate }: { navigate: (path: string) => void }) {
  ...
  return <main className="showcase-page e54" style={{ "--bg": tokens.background, "--surface": tokens.surface, "--text": tokens.text, "--muted": tokens.muted, "--primary": tokens.primary } as CSSProperties}>
