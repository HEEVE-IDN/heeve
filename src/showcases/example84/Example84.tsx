...
import "./example84.css";
import Catalog from "./Catalog";
import { useTheme } from "../../hooks/useTheme";
import type { ShowcaseConfig } from "../../types/showcase";

const data = config as ShowcaseConfig & Record<string, any>;
export default function Example84({ navigate }: { navigate: (path: string) => void }) {
  ...
  return <main className="store-showcase" style={{ "--bg": t.background, "--surface": t.surface, "--text": t.text, "--muted": t.muted, "--primary": t.primary } as React.CSSProperties}>
    ...
    <section className="showcase-collection" id="collection" aria-labelledby="collection-title"><header><div><p className="eyebrow">{data.collection.eyebrow}</p><h2 id="collection-title">{data.collection.title}</h2></div><p>{data.collection.description}</p></header><Catalog products={data.products} slug={data.slug} navigate={navigate} whatsapp={data.contactPersonWhatsapp} purchaseLabel={data.collection.purchaseLabel} detailLabel={data.collection.detailLabel} /></section>
