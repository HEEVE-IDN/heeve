...
export default function ProductPage({ config, product, navigate }: { config: ShowcaseConfig; product: Product; navigate: (path: string) => void }) {
  ...
  return <main className="product-page" style={{ "--bg": tokens.background, "--surface": tokens.surface, "--text": tokens.text, "--muted": tokens.muted, "--primary": tokens.primary } as React.CSSProperties}>
    ...
    <div className="product-layout"><div className="product-visual"><LocalGallery product={product} /></div><section aria-labelledby="product-title"><p className="eyebrow">{product.category} / {product.variant}</p><h1 id="product-title">{product.name}</h1><p>{product.description}</p><strong>{data.ui.unitPrefix} {product.unit}</strong><a href={contact} target="_blank" rel="noreferrer">{data.ui.contactAction}</a></section></div>
  </main>;
}
