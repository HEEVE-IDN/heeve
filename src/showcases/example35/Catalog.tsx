...
export function Catalog({ products, slug, navigate, whatsapp, purchaseLabel, detailLabel }: CatalogProps) {
  ...
  return <div className="e35-local-catalog">
    {categories.length > 1 && <div className="e35-local-catalog-filter" aria-label="Product categories">
    ...
    <div className="e35-local-catalog-grid">
      {shown.map((product) => {
        ...
        return <article className="e35-local-product-card" key={product.number}>
          <button type="button" className="e35-local-product-open" onClick={() => openProduct(product)} aria-label={`${detailLabel} ${product.name}`}>
            <span className="e35-local-product-art">{product.photoUrls[0] ? <img src={product.photoUrls[0]} alt={product.name} /> : <span>{String(product.number).padStart(2, "0")}</span>}</span>
            <span className="e35-local-product-copy"><small>{product.category} · {product.unit}</small><strong>{product.name}</strong>{product.variant && <em>{product.variant}</em>}<span>{product.description}</span></span>
          </button>
          <div className="e35-local-product-actions">
